...
class GitHubGraphExportError(RuntimeError):
    """Raised when a GitHub graph export fails."""
...
def ensure_author(
    ...
    *,
    ...
    if source and source not in current["source"].split(","):
...
def run_gh_graphql(query: str, variables: dict[str, str]) -> dict[str, Any]:
    ...
    if result.returncode != 0:
        ...
        raise GitHubGraphExportError(f"gh graphql failed: {message}")
...
def run_gh_api(endpoint: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if result.returncode != 0:
            message = result.stderr.strip() or result.stdout.strip() or "unknown gh error"
            raise GitHubGraphExportError(f"gh api failed for {paged_endpoint}: {message}")
...
def default_output_dir(repo: str) -> Path:
    return Path("outputs/github-graph") / repo.replace("/", "__")
...
def export_repo_graph(
    ...
    *,
    ...
    resolved_output_dir = Path(output_dir) if output_dir else default_output_dir(repo)

    if progress_callback is not None:
        progress_callback("fetch_prs", 1, 4, f"Fetching pull requests for {repo}")
    prs = fetch_prs(repo)

    if progress_callback is not None:
        progress_callback("fetch_commits", 2, 4, f"Fetching commits for {repo}")
    commits = fetch_commits(repo)

    if progress_callback is not None:
        progress_callback("normalize", 3, 4, "Normalizing graph data")
    graph = normalize_data(repo, prs, commits)

    if progress_callback is not None:
        progress_callback("write_outputs", 4, 4, "Writing CSV and JSON outputs")
    write_outputs(resolved_output_dir, repo, prs, commits, graph)
